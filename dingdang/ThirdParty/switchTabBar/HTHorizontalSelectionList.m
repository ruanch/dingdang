...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _HTHorizontalSelectionListInternalPadding = kHTHorizontalSelectionListInternalPadding;
        _HTHorizontalSelectionListHorizontalMargin = kHTHorizontalSelectionListHorizontalMargin;
...
- (void)reloadData {
    ...
    for (NSInteger index = 0; index < totalButtons; index++) {
        ...
        UIButton *button = [self.dataSource selectionList:self titleForItemWithIndex:index];//[self selectionListButtonWithTitle:buttonTitle];
        [self.contentView addSubview:button];

        if (previousButton) {
            [self.contentView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:[previousButton]-padding-[button]"
                                                                                     options:NSLayoutFormatDirectionLeadingToTrailing
                                                                                     metrics:@{@"padding" : @(_HTHorizontalSelectionListInternalPadding)}
            ...
            [self.contentView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-margin-[button]"
                                                                                     options:NSLayoutFormatDirectionLeadingToTrailing
                                                                                     metrics:@{@"margin" : @(_HTHorizontalSelectionListHorizontalMargin)}
...
- (UIButton *)selectionListButtonWithTitle:(NSString *)buttonTitle {
    ...
    if ([buttonTitle isEqualToString:@"...."]) {
        [button setImage:[UIImage imageNamed:@"pull_icon"] forState:UIControlStateNormal];
        [button addTarget:self
                   action:@selector(moreButtonWasTapped:)
...
- (void) moreButtonWasTapped:(id)sender
{
